...
SCHEDULER.every '60m', :first_in => 0 do |job|
  ...
  for bug in buglist["bugs"]
    ...
    summary = bug["summary"]
    if resolved_time > created_time and (summary.include?('stage') or summary.include?('staging'))
  ...
  send_event('deploy_cycle_stage', { current: avg_deploy_time.round(2), last: last_avg })
end
